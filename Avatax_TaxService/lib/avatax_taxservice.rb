...
module AvaTax
  ...
  class TaxService
    ...
    def posttax(document)
      ...
      #Strip off outer layer of the hash - not needed
      #Return data to calling program
      @response =  messages_to_array(@response[:post_tax_response][:post_tax_result])
      return @response
      #Capture unexpected errors
      rescue Savon::Error => error
        abend(error)
    ...
    def messages_to_array(response)
      ...
      #if there's only one error (or no errors), they'll be floating around at the root
      if response[:messages].nil?
        ...
        # if there's more than one message, they're inconveniently nested.
      elsif response[:messages][:message].kind_of?(Array) 
        pretty_messages = Array.new
        response[:messages][:message].each do |msg|
          pretty_messages << msg          
        end
        response[:messages] = pretty_messages
      else
        response[:messages] = [response[:messages][:message]]
      end
      return response
    end
  end
